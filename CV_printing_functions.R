...
create_network_object <-  function(data_location,
                              pdf_mode = FALSE,
                              sheet_is_publicly_readable = TRUE) {
  ...
  if(is_google_sheets_location){
    ...
    # Want to go old-school with csvs?
    cv$entries_data <- readr::read_csv(paste0(data_location, "network_entries.csv"), skip = 1)
    cv$skills       <- readr::read_csv(paste0(data_location, "language_skills.csv"), skip = 1)
    cv$text_blocks  <- readr::read_csv(paste0(data_location, "text_blocks.csv"), skip = 1)
    cv$contact_info <- readr::read_csv(paste0(data_location, "contact_info.csv"), skip = 1)
    cv$abstracts    <- readr::read_csv(paste0(data_location, "abstracts.csv"), skip = 1)
...
print_wordcloud <- function(my_data) {
  ...
  filter <- c("research_positions", "education", "conferences")
  
  section_data <- dplyr::filter(my_data$entries_data, section %in% filter)
  
  abstracts <- my_data$abstracts
  ...
  wordcloud(words = d$word, freq = d$freq, min.freq = 1,
            ...
            colors=brewer.pal(8, "Blues")[c(4:8)])
}

print_language_skills <- function(cv, out_of = 5, bar_color = "#969696", bar_background = "#d9d9d9", glue_template = "default"){
...
build_network_logo2 <- function (position_data) 
{
  positions <- position_data %>% dplyr::mutate(id = dplyr::row_number(), 
                                               ...
                                               section = stringr::str_replace_all(section, "_", " ") %>% 
  ...
  edges <- positions %>% dplyr::select(id, start_year, end_year) %>% 
    dplyr::mutate(end_year = ifelse(end_year > current_year, 
                                    current_year, end_year), start_year = ifelse(start_year > 
                                                                                   current_year, current_year, start_year)) %>% purrr::pmap_dfr(function(id, 
                                                                                                                                                         start_year, end_year) {
                                                                                     dplyr::tibble(year = start_year:end_year, id = id)
                                                                                   }) %>% dplyr::group_by(year) %>% tidyr::nest() %>% dplyr::rename(ids_for_year = data) %>% 
    purrr::pmap_dfr(function(year, ids_for_year) {
      combination_indices(nrow(ids_for_year)) %>% dplyr::transmute(year = year, 
                                                                   source = ids_for_year$id[a], target = ids_for_year$id[b])
    })
